...
} from "./prospectivePublishedModelEvaluation";
import {
...
export type RandomClockProspectiveEvaluationReport = {
  schemaVersion: "prospective-random-clock-model-evaluation-v1";
  status:
    | "insufficient_data"
    | "promising"
  ...
  targetDefinition: typeof RANDOM_ELAPSED_SHADOW_TARGET_DEFINITION;
  activeModelVersion: typeof RANDOM_ELAPSED_SHADOW_MODEL_VERSION;
  baselineModelVersion: typeof PUBLISHED_PROBABILITY_MODEL_VERSION;
  freezeAt: typeof RANDOM_ELAPSED_SHADOW_FREEZE_AT;
...
export function selectComparableRandomClockForecasts(rows: Array<ProspectiveForecastRow>) {
  return rows.filter((row) =>
    isStoredForecast(row.forecasts[RANDOM_ELAPSED_SHADOW_MODEL_VERSION])
    && isStoredForecast(row.forecasts[PUBLISHED_PROBABILITY_MODEL_VERSION]),
  );
}

export function selectDailyFirstRandomClockForecasts(rows: Array<ProspectiveForecastRow>) {
  return selectDailyFirstForecasts(selectComparableRandomClockForecasts(rows));
}

export function getRandomClockOutcome(
  boundaries: Array<RecoveryResetBoundary>,
  origin: string,
  horizonHours: number,
): boolean | null {
  const originTime = timestamp(origin);
  if (originTime === null || !Number.isFinite(horizonHours) || horizonHours <= 0) return null;
  ...
  }).sort((left, right) => timestamp(left.resetAt)! - timestamp(right.resetAt)!);
  const firstBoundaryTime = timestamp(withinHorizon[0]?.resetAt);
  if (firstBoundaryTime === null) return false;
  const firstBoundaryGroup = withinHorizon.filter(
    (boundary) => timestamp(boundary.resetAt) === firstBoundaryTime,
  );
  if (firstBoundaryGroup.some((boundary) => boundary.isRandom)) return true;
  if (firstBoundaryGroup.some((boundary) => boundary.isRegular)) return null;
  return false;
}
...
function getResolvedRows(
  ...
  return rows.flatMap((row) => {
    ...
    const originTime = generatedTime;
...
export function evaluateRandomClockModelProspectively(
  rows: Array<ProspectiveForecastRow>,
  boundaries: Array<RecoveryResetBoundary>,
  asOf: Date,
  ...
  return {
    ...
    activeModelVersion: RANDOM_ELAPSED_SHADOW_MODEL_VERSION,
    baselineModelVersion: PUBLISHED_PROBABILITY_MODEL_VERSION,
    freezeAt: RANDOM_ELAPSED_SHADOW_FREEZE_AT,
    evaluationStartAt,
    forecastCounts: {
      active: eligibleRows.filter((row) => isStoredForecast(row.forecasts[RANDOM_ELAPSED_SHADOW_MODEL_VERSION])).length,
      baseline: eligibleRows.filter((row) => isStoredForecast(row.forecasts[PUBLISHED_PROBABILITY_MODEL_VERSION])).length,
    ...
    notes: [
      ...
      `The random-clock shadow parameters are frozen at ${RANDOM_ELAPSED_SHADOW_FREEZE_AT}; ${RANDOM_ELAPSED_SHADOW_FREEZE_POLICY}`,
    ],
  };
}
