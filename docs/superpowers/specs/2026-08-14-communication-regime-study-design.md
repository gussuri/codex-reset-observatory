...
- 古いeventでsignal coverageが不足している場合、signal不在だけを理由に`silent`へ再分類しない。`observed_signal`と`legacy_history`のprovenanceを保持する。
- non-reset exposureの分母は`tibo_heartbeat`等でcoverage confirmedな時間binだけに限定し、scan failure/coverage不明期間はnone exposureに数えない。
...
## Communication classification and legacy labels

eventのcompleted timeより前、かつ前回eligible random resetより後のsignalだけを候補にする。実際のsignal coverageが信頼できるeventでは、availableなactual signalからprimary typeを再構成する。coverage不足のhistorical eventでは、`LOCAL_RESET_HISTORY.details.noticeType`をlegacy labelとして使用する。

- provenanceは`observed_signal`または`legacy_history`とし、両方ある場合はobserved signalをprimaryにしてlegacyとの一致/不一致をauditする。
- `official_notice`/`teaser`の有効性は既存Productionのsignal validity policyをprimaryに再利用する。研究用confidence cutoffをcanonical truthにしない。cutoffが必要な感度分析では別集計に分け、threshold変更による件数・遷移・shadow性能の変化を報告する。
- observed signalでformal noticeが1件以上なら`formal_notice`、formal noticeがなくteaserが1件以上なら`teaser`、validな事前signalがない場合だけ`silent`とする。
- legacy `opened_at`をsignal timestampに使うのは、その値が事前通信時刻を表すことを確認できたeventだけに限定する。意味が不明な場合は`signalToExecutionHours`を計算しない。
...
`signalが存在しない`と`Tibo監視ができていない`を区別する。既存の`tibo_heartbeat`等のmonitoring-healthをread-only取得し、coverage confirmedの時間binだけをnon-reset exposureの分母へ入れる。coverage不明、scan failure、page reload failureの期間はsilent/none exposureへ数えない。coverage policy、coverage source、confirmed interval、excluded interval、coverage confidenceをreportへ記録する。
...
prequential/rolling point-in-time評価を主評価とする。LOOはsmall-nで構造が安定しているかを見る補助診断に限定する。LOOでfuture eventをparameter estimationへ使う場合、その結果をprospective、point-in-time、OOSとは表現しない。
